...
export interface SysTenantListQuery {
  page: number;
  pageSize: number;
  tenantId: string;
...
export interface SysTenantPackageListQuery {
  page: number;
  pageSize: number;
  tenantId: string;
  contactUserName: string;
...
export async function getSysTenantListApi(params: SysTenantListQuery) {
  return requestClient.get<SysTenantListRes>('/system/tenant/list', { params });
}

export async function getSysTenantPackageListApi(params: SysTenantPackageListQuery) {
