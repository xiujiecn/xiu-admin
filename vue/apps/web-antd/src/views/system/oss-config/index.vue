...
import { getSysOssConfigListApi, deleteSysOssConfigApi } from '#/api/system/oss-config';
import {
  MdiPlus,
  MdiDelete,
} from '@vben/icons';
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        ...
        return await getSysOssConfigListApi({
...
function handlePreview(record: SysOssConfigListData) {
...
function handleEdit(row: SysOssConfigListData) {
  editDrawerApi.setData({ id: row.ossConfigId, update: true, view: false });
  editDrawerApi.open();
}

async function handleDelete(row: SysOssConfigListData) {
  await deleteSysOssConfigApi({ ossConfigIds: [row.ossConfigId] });
...
function handleMultiDelete() {
  ...
  const ids = rows.map((row: SysOssConfigListData) => row.ossConfigId);
  Modal.confirm({
    title: '提示',
    okType: 'danger',
    content: `确认删除选中的${ids.length}条记录吗？`,
    onOk: async () => {
      await deleteSysOssConfigApi({ ossConfigIds: ids });
      message.success("删除成功");
      await handleRefresh();
    },
  });
...
<template>
  <Page auto-content-height>
    <Grid table-title="OSS配置列表">
      <template #toolbar-tools>

        <Button class="mr-2 flex items-center " type="primary" :icon="h(MdiPlus)" @click="handleAdd" v-access:code="'cpm:system:ossConfig:add'">新增</Button>
        <Button class="mr-2 flex items-center" type="primary" disabled :icon="h(MdiDelete)"
          @click="handleMultiDelete" v-access:code="'cpm:system:ossConfig:remove'">删除</Button>
      </template>
