...
import { addSysOssConfigApi, editSysOssConfigApi, getSysOssConfigViewApi } from '#/api/system/oss-config';
...
const [BasicDrawer, drawerApi] = useVbenDrawer({
  ...
  async onOpenChange(isOpen) {
    ...
    if (isUpdate.value || isView.value) {
      const record = await getSysOssConfigViewApi({ ossConfigId: Number(id) });
...
async function handleConfirm() {
  try {
    ...
    await (isUpdate.value ? editSysOssConfigApi (data) : addSysOssConfigApi(data));
    emit('reload');
    await handleCancel();
  } catch (error) {
    console.error(error);
