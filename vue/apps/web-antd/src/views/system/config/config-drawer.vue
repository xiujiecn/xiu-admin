...
import { addSysConfigApi, editSysConfigApi, getSysConfigByIdApi } from '#/api/system/config';
...
const [BasicDrawer, drawerApi] = useVbenDrawer({
  ...
  async onOpenChange(isOpen) {
    ...
    if (isUpdate.value || isView.value) {
      const record = await getSysConfigByIdApi({ configId: Number(id) });
...
async function handleConfirm() {
  try {
    ...
    await (isUpdate.value ? editSysConfigApi (data) : addSysConfigApi(data));
    emit('reload');
    await handleCancel();
  } catch (error) {
    console.error(error);
