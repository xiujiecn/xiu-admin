...
func (manager *ClientManager) start() {
	...
	for {
		...
		case message := <-manager.TagBroadcast:
			...
			// g.Log().Debugf(context.TODO(), "client_manager.go 标签广播事件 find:%v msg:%v", find, message)
...
func SendToUser(userID uint64, response *WResponse) {
	...
	// g.Log().Debugf(context.Background(), "websocket SendToUser userID:%v response:%+v", userID, response)
}
