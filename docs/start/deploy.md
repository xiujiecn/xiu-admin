...
gfcli:
  build:
    ...
    version: ""
    output: "./temp/xiuadmin"                     # 可执行文件生成路径
    extra: ""
```
...
cd ../vue && pnpm run build                               # 切换到web项目下，编译web项目
\cp -rf ./dist/*  ../server/resource/public/admin/    # 将编译好的web资源复制到server对应的资源存放路径下
cd ../server                                          # 切换回服务端目录下
echo "y" | gf build                                   # 编译XiuAdmin服务端

# 不出意外你已经编译好了xiuadmin可执行文件！
```
...
# web端编译完成后，将vue/dist/*中的文件上传到`server`端线上运行目录：/resource/public/admin即可
